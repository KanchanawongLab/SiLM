...
//fit psf with maximum likelihood error using l-m algorithm, only one sigma are output
...
__global__ void kernel_gpustorm_fitsigmamle( float *data,  float *fitresult, float *fiterror,const int iterations, const int sz, const float psfSigma){
