...
// fit psf with maximum likelihood error using l-m algorithm, sigmax and sigmay are output
...
__global__ void kernel_gpustorm_fitsigmaxymle(float *data,  float *fitresult, float *fiterror,const int iterations, const int sz, const float psfSigma){
	...
	float theta[NV];
	float maxjump[NV]={1.0f,1.0f, 100.0f, 2.0f, 0.5f,0.5f};
	float gamma[NV]={1.0f,1.0f, 0.5f, 1.0f,1.0f,1.0f};
	...
	float new_update[NV]={1e13f,1e13f,1e13f,1e13f,1e13f,1e13f};
	float old_update[NV];
	float old_theta[NV];
	...
	for (kk1=0;kk1<iterations;kk1++)
	{
	  ...
	  memset(M_hession,0,NV*NV*sizeof(float));
		...
		for (m=0; m<sz;m++) {for (mm=0;mm<sz;mm++) {
			...
			kernel_DerivativeGauss2D_sigmaxy(m,mm,theta,dudt,&model);

			datai=sdata[sz*mm+m];
			
			//switch between mle and lse fitting
			if (model>10e-3){
				t1=1-datai/model;
       ...
       for (p=0; p<NV;p++)
		...
		theta[5] = kernel_cumax(theta[5],0.5);
		theta[5] = kernel_cumin(theta[5],sz/2.0f);
    ...
    fiterror[(tid)*7+0]=new_err;
	for(m=0;m<NV;m++)
	{
		fiterror[(tid)*7+m+1]=diagMcrlb_inv[m];
	}
